    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {suppliers.map((supplier) => (
          <div
            key={supplier.id}
            className="glass-card p-6 sm:p-8 rounded-3xl border border-slate-200 flex flex-col sm:flex-row items-start space-y-4 sm:space-y-0 sm:space-x-6 hover:border-white/40 transition-all"
          >
            <div className="w-20 h-20 rounded-2xl bg-white overflow-hidden flex-shrink-0 border border-slate-200 flex items-center justify-center p-2">
              {supplier.logo ? (
                <img src={supplier.logo} alt={supplier.name} className="w-full h-full object-contain" />
              ) : (
                <Factory className="w-8 h-8 text-slate-950" />
              )}
            </div>

            <div className="space-y-3 flex-1">
              <div>
                <span className="text-[10px] font-extrabold uppercase px-2 py-0.5 rounded bg-slate-900/20 text-slate-950 font-heading">
                  {supplier.category}
                </span>
                <h3 className="text-xl font-bold text-slate-950 mt-1.5 font-heading">
                  {supplier.name}
                </h3>
              </div>
              ...
              <div className="pt-2 flex items-center justify-between text-xs text-slate-600 border-t border-slate-200">
                ...
                {supplier.website && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex items-center space-x-1 font-bold text-slate-950 hover:underline font-heading"
                  >
