          ...
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-bold whitespace-nowrap transition-all ${
              filterCat === cat
                ...
                : 'white slate-700 hover:slate-100 border slate-200'
            }`}
          >
            {cat}
          </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {filtered.map((blog) => (
          <div
            ...
            className="glass-card rounded-3xl overflow-hidden cursor-pointer flex flex-col justify-between group hover:slate-900/50 transition-all duration-300"
          >
            <div className="aspect-[16/9] white overflow-hidden relative">
              <img
                src={blog.image || '/hero-collage.jpg'}
                alt={blog.title}
                className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"
              />
              <div className="absolute top-3 left-3 px-3 py-1 rounded-full text-[10px] font-bold uppercase slate-950 white font-heading">
                {blog.category}
              </div>
            </div>
            ...
            <div className="p-6 space-y-4 flex-1 flex flex-col justify-between">
              <div className="space-y-2">
                <div className="flex items-center space-x-2 text-[11px] slate-600">
                  <Calendar className="w-3.5 h-3.5 slate-950" />
                  <span>{blog.publishedAt}</span>
                  <span>•</span>
                  <User className="w-3.5 h-3.5 slate-600" />
                  <span>{blog.author}</span>
                </div>

                <h3 className="text-lg font-bold slate-950 group-hover:slate-950 transition-colors font-heading leading-snug">
                  {blog.title}
                </h3>

                <p className="text-xs slate-600 line-clamp-3 leading-relaxed">
                  {blog.summary}
                </p>
              </div>

              <div className="pt-4 border-t slate-200/80 flex items-center justify-between text-xs font-bold slate-950 font-heading">
